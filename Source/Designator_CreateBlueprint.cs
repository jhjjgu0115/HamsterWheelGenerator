...
namespace Blueprints
{
    public class Designator_CreateBlueprint : Designator
    {
        ...
        public Designator_CreateBlueprint()
        {
            ...
            defaultLabel = "Nuka.Blueprints.Create".Translate();
            defaultDesc = "Nuka.Blueprints.CreateHelp".Translate();
            useMouseIcon = true;
            ...
            get
            {
                ...
                foreach (var file in Controller.GetSavedFilesList())
                {
                    ...
                    if (Controller.FindBlueprint(name) == null)
                        options.Add(new FloatMenuOption("Nuka.Blueprints.LoadFromXML".Translate(name),
                            delegate { Controller.Add(Controller.LoadFromXML(file.Name)); }));
                }

                if (options.NullOrEmpty())
                    Messages.Message("Nuka.Blueprints.NoStoredBlueprints".Translate(), MessageTypeDefOf.RejectInput);
                return options;
            }
        }
        ...
        public override void DesignateMultiCell( IEnumerable<IntVec3> cells )
        {
            ...
            {
                Messages.Message( "Nuka.Blueprints.CannotCreateBluePrint_NothingSelected".Translate(),
